...
export default function HeroNav() {
  ...
  return (
    <nav
      class="relative mx-4 md:mx-auto md:max-w-screen-md rounded-lg"
      style="backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); background: rgba(255, 255, 255, 0.08); border: 1px solid rgba(255, 255, 255, 0.12);"
    >
      {/* Desktop */}
      <div class="hidden md:flex mx-auto max-w-screen-md py-3 flex-row items-center justify-center gap-1">
        {navLinks.map((l) => (
          <a
            href={l.href}
            class="px-3 py-2 rounded-md text-sm font-medium tracking-wide uppercase text-white hover:bg-white hover:bg-opacity-10 transition duration-200"
          >
            {l.label}
          </a>
        ))}
