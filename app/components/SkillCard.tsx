...
export function SkillCard(props: Props) {
  const isOpen = props.selectedCard === props.skill.name &&
    props.selectedCardState;
  return (
    <a
      ...
      href={"#skill_" +
        props.skill.name.toLowerCase().replace("/", "_").replace(" ", "_")}
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
        props.setSelectedCard(props.skill.name);
        props.setSelectedCardState(
          e.currentTarget.children[1].classList.contains("hidden"),
        );
        props.setClickedTime(Date.now());
      }}
    >
      <span class="text-sm font-medium align-middle dark:text-gray-700">
        {props.skill.icon_data}
        {props.skill.name}
      </span>
      <div
        id={"skill_" +
